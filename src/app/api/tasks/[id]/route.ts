...
export async function GET(req: NextRequest, { params }: { params: Promise<TaskParams> }) {
    try {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<TaskParams> }) {
    try {
        const { userId } = await auth();
        if (!userId) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<TaskParams> }) {
    try {
        const { userId } = await auth();
        // if (!userId) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
